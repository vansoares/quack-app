...
// só essa conta enxerga a página de admin — configurável por variável de
// ambiente, mas com um valor padrão pra já funcionar sem configuração extra
const ADMIN_EMAIL = (process.env.ADMIN_EMAIL || "vanessa.ssoares.vs@gmail.com").trim().toLowerCase();
...
function userPublic(u){
  return {
    ...
    features: Object.keys(u.features || {})
  };
}

function requireAdmin(req, res, next){
  if(!isAdminUser(req.user)) return res.status(403).json({ error: "acesso restrito" });
  next();
}
...
app.get("/api/admin/users", requireAuth, requireAdmin, h(async function(req, res){
  var users = await db.listUsers();
  res.json({ users: users.map(adminUserView) });
}));

app.get("/api/admin/features", requireAuth, requireAdmin, h(async function(req, res){
  var features = await db.listFeatureDefs();
  res.json({ features: features });
}));

app.post("/api/admin/features", requireAuth, requireAdmin, h(async function(req, res){
  var key = (req.body && req.body.key || "").trim().toLowerCase();
  var label = (req.body && req.body.label || "").trim();
  try{
    var features = await db.addFeatureDef(key, label);
    res.json({ features: features });
  }catch(e){
    if(e.message === "CHAVE_INVALIDA"){
      return res.status(400).json({ error: "Use de 2 a 40 letras minúsculas, números, _ ou -." });
    }
    if(e.message === "FEATURE_EXISTENTE"){
      return res.status(409).json({ error: "Já existe uma feature com essa chave." });
    }
    throw e;
  }
}));

app.delete("/api/admin/features/:key", requireAuth, requireAdmin, h(async function(req, res){
  var features = await db.removeFeatureDef(String(req.params.key || "").trim().toLowerCase());
  res.json({ features: features });
}));

app.post("/api/admin/users/:id/features", requireAuth, requireAdmin, h(async function(req, res){
  var key = (req.body && req.body.key || "").trim().toLowerCase();
  var enabled = !!(req.body && req.body.enabled);
  try{
    var ok = await db.setUserFeature(req.params.id, key, enabled);
    if(!ok) return res.status(404).json({ error: "Usuário não encontrado." });
    res.json({ ok: true });
  }catch(e){
    if(e.message === "CHAVE_INVALIDA"){
      return res.status(400).json({ error: "Chave de feature inválida." });
    }
    throw e;
  }
}));
