...
// as mesmas cores categóricas já usadas nos ícones das ferramentas, aplicadas
// ao mesmo patinho da marca — nenhum asset novo, só a paleta que já existe
var AVATARS = ["signal", "rest", "amber", "teal", "clover", "clay"];

function userPublic(u){
  return { email: u.email, username: u.username || null, avatar: u.avatar || null };
}
...
app.post("/api/auth/login", h(async function(req, res){
  ...
  // mesmo sem usuário, roda um bcrypt.compare (contra um hash fixo, sem
  // relação com senha nenhuma) pra gastar o mesmo tempo do caminho onde o
  // usuário existe — sem isso, dava pra descobrir quais e-mails têm conta
  // só medindo quanto tempo a resposta demora
  var ok = await bcrypt.compare(String(password), user ? user.passwordHash : DUMMY_HASH);
  if(!user || !ok) return invalido();
  setSessionCookie(req, res, signSession(user.id));
  res.json(userPublic(user));
}));
...
app.post("/api/auth/avatar", requireAuth, h(async function(req, res){
  var avatar = req.body && req.body.avatar;
  if(AVATARS.indexOf(avatar) < 0) return res.status(400).json({ error: "Patinho inválido." });
  await db.setAvatar(req.user.id, avatar);
